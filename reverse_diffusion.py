...
def main():
    ...
    with dnnlib.util.open_url(network_pkl) as f:
        net = pickle.load(f)['ema'].to(device)
    # 前向扩散过程（ODE逆向）
    x0_noise, _ = ode_forward_diffusion(x0, net, num_steps=2, sigma_min=0.002, sigma_max=20, rho=7, device=torch.device('cuda'))
    save_image(x0_noise[-1], 'x0_noise.png')
    save_image_channel(x0_noise[-1], 'x0_noise_channel.png')
    save_image_grid(x0_noise, 'x0_noise_grid.png', grid_size=(4, 5))
    reconstructed = ode_reverse_diffusion(x0_noise[-1], net, num_steps=18, sigma_min=0.002, sigma_max=20, rho=7, device=torch.device('cuda'))
    save_image(reconstructed[-1], 'reconstructed.png')
    print(torch.max(reconstructed[-1].detach().cpu()-x0.detach().cpu()))
    save_image(reconstructed[-1].detach().cpu()-x0.detach().cpu(), 'reconstructed_channel.png')
    save_image_grid(reconstructed, 'reconstructed_grid.png', grid_size=(4, 5))
